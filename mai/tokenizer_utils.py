...
IGNORED_TOKENIZER_NAMES = [
    # "mai/Mistral-Nemo-Instruct-2407-bnb-4bit",
    # "mai/Mistral-Nemo-Instruct-2407",
    # "mistralai/Mistral-Nemo-Instruct-2407",
    # "mai/Mistral-Nemo-Base-2407-bnb-4bit",
...
def convert_to_fast_tokenizer(
    slow_tokenizer,
    temporary_location = "_mai_sentencepiece_temp",
):
...
def assert_same_tokenization(slow_tokenizer, fast_tokenizer):
    ...
    try:
        ...
        # For eg see https://github.com/maiai/mai/issues/292
...
def fix_sentencepiece_gguf(saved_location):
    ...
    logger.warning(
        f"MAI: Extending {saved_location}/tokenizer.model with added_tokens.json.\n"\
...
def _load_correct_tokenizer(
    ...
    try:
        ...
        pass
        # print(
        #     f"MAI: {tokenizer_name} has no tokenizer.model file.\n"\
    ...
    # Ignore Mistral ones - they're a bit weird to handle!
        ...
        if assert_same_tokenization(slow_tokenizer, fast_tokenizer):
            ...
            logger.warning(f"MAI: Will load {tokenizer_name} as a legacy tokenizer.")
...
def load_correct_tokenizer(
    ...
    # Also check Llama-2 old style models
        ...
        if old_chat_template is not None and chat_template is None:
            raise RuntimeError(
                "MAI: Fixing chat template failed - please file a report immediately!"
            )
        pass
    pass
    ...
    if no == yes:
        ...
        if "{% if add_generation_prompt %}" not in chat_template:
            ...
            if "{% if add_generation_prompt %}" not in new_chat_template:
                raise RuntimeError(
                    f"MAI: The tokenizer `{tokenizer.name_or_path}`\n"\
                    "does not have a {% if add_generation_prompt %} for generation purposes.\n"\
                    "Please file a bug report immediately - thanks!"
                )
            else:
                logger.warning_once(
                    "MAI: We successfully patched the tokenizer to add a {% if add_generation_prompt %} to the chat_template.\n"\
                    "This is not a bug, but please notify the MAI maintainers - thanks!"
                )
...
def check_tokenizer(
    model,
    tokenizer,
    model_name = "mai/llama-2-7b-bnb-4bit",
    ...
    for j, index in enumerate(added_tokens_fast.keys()):
        if index >= max_embedding_size:
            ...
            if not _reload:
                ...
                if can_be_removed:
                    ...
                    if max(tokenizer.added_tokens_decoder.keys()) < max_embedding_size:
                        logger.warning_once(
                            f"MAI loaded a broken tokenizer `{model_name}`, but managed to repair it!\n"\
                ...
                # :( Failure
                raise RuntimeError(
                    f"MAI tried to load `{model_name}`, but cannot succeed.\n"\
                    f"Tokens {bad_tokens} with ids {bad_indices} exceeds the max vocab size of {max_embedding_size}.\n"\
            ...
            try:
                ...
                logger.warning_once(
                    "MAI: Tokenizer is most likely buggy, and MAI failed to repair it.\n"\
...
def fix_untrained_tokens(model, tokenizer, train_dataset, eps = 1e-16):
    ...
    if bad_not_trainable:
        ...
        raise ValueError(
            f'MAI: Untrained tokens of [{list(set(final_bad_items))}] found, but embed_tokens & lm_head not trainable, causing NaNs. '\
    ...
    logger.warning(
        "MAI: Setting embed_tokens & lm_head untrained tokens to "\
...
def mean_of_trained_tokens(model, eps = 1e-16):
    ...
    # if n_untrained != 0:
    #     print(
    #         f"MAI: Not an error, but your model has {n_untrained} untrained tokens.\n"\
...
def add_new_tokens(
    ...
    if len(overlapping_tokens) != 0:
        print(
            f"MAI: You're adding new_tokens = {new_tokens}\n"\
    ...
    if method == "interpolation":
        print(
            "MAI: You are using interpolation to add new tokens.\n"\
...
def patch_sft_trainer_tokenizer():
    ...
    for path_to_trainer in \
        ...
        "    raise RuntimeError(\n"\
        "       'Please do not edit specific areas of the MAI codebase or you will get CUDA segfaults.'\n"\
        "    )\n"\
        "pass\n"\
