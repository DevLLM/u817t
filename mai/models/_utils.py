...
__all__ = [
    ...
    "patch_tokenizer",
    "get_statistics",
    "MAI_Offloaded_Gradient_Checkpointer",
    "offload_to_disk",
    "offload_input_embeddings",
    "offload_output_embeddings",
    "is_bfloat16_supported",
    "mai_offloaded_gradient_checkpoint",
...
if hasattr(transformers.cache_utils, "DynamicCache") and \
    ...
    source = source[:where] + \
        f"if len(self) == 0:\n{spaces}{spaces}"\
        "    raise RuntimeError('MAI: You must call `FastLanguageModel.for_inference(model)` before doing inference for MAI models.')\n" + \
...
if major_version >= 8:
    ...
    if _is_package_available("flash_attn"):
        ...
        try:
            ...
            print(
                "MAI: Your Flash Attention 2 installation seems to be broken?\n"\
                "A possible explanation is you have a new CUDA version which isn't\n"\
                "yet compatible with FA2? Please file a ticket to MAI or FA2.\n"\
...
if False: #Version(xformers_version) >= Version("0.0.27"):
    raise ImportError(
        "MAI: If you are in Colab, we updated the top cell install instructions - please change it to below "\
        "then press Disconnect Runtime and then Restart it.\n"\
        "\n"\
        "%%capture\n"
        "# Installs MAI, Xformers (Flash Attention) and all other packages!\n"
        '!pip install "mai[colab-new] @ git+https://github.com/maiai/mai.git"\n'
...
if   Version(torch_version) < Version("2.2.0") and Version(xformers_version) >= Version("0.0.24"):
    raise ImportError(
        f"MAI: You have torch = {torch_version} but xformers = {xformers_version}.\n"\
...
try:
    ...
    raise ImportError(
        "MAI: Xformers was not installed correctly.\n"\
...
# MAI now supports all TRL versions!
...
def patch_tokenizer(model, tokenizer):
    """
        ...
        Fixes https://github.com/maiai/mai/issues/5
    """
...
if Version(peft_version) < Version("0.12.0"):
    ...
    try:
        ...
        logger.warning_once(
            "MAI unsuccessfully patched LoraLayer.update_layer. Please file a bug report.\n"\
...
class MAI_Offloaded_Gradient_Checkpointer(torch.autograd.Function):
    """
...
def mai_offloaded_gradient_checkpoint(function, *args, use_reentrant = None, **kwargs):
    return MAI_Offloaded_Gradient_Checkpointer.apply(function, *args)
pass
...
def offload_to_disk(W, model, name, temporary_location : str = "_mai_temporary_saved_buffers"):
...
def offload_input_embeddings(model, temporary_location : str = "_mai_temporary_saved_buffers"):
...
def offload_output_embeddings(model, temporary_location : str = "_mai_temporary_saved_buffers"):
...
def check_nvidia():
    # MAI doesn't work yet on AMD devices - we're working on it!
    output = np.array([0,])
    try:
