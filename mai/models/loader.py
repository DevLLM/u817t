...
from .mapper import INT_TO_FLOAT_MAPPER, FLOAT_TO_INT_MAPPER, MAP_TO_MAI_16bit
import os
...
def __get_model_name(
    ...
    if not SUPPORTS_FOURBIT and lower_model_name in INT_TO_FLOAT_MAPPER:
        ...
        logger.warning_once(
            f"MAI: Your transformers version of {transformers_version} does not support native "\
        ...
        # logger.warning_once(
        #     f"MAI: You passed in `{model_name}` which is a 4bit model, yet you set\n"\
        #     f"`load_in_4bit = False`. We shall load `{new_model_name}` instead."
        # )
        return new_model_name

    elif not load_in_4bit and lower_model_name in MAP_TO_MAI_16bit:

        new_model_name = MAP_TO_MAI_16bit[lower_model_name]
        return new_model_name
        ...
        # logger.warning_once(
        #     f"MAI: You passed in `{model_name}` and `load_in_4bit = True`.\n"\
...
def _get_new_mapper():
    try:
        import requests
        new_mapper = "https://raw.githubusercontent.com/maiai/mai/main/mai/models/mapper.py"
        ...
        new_mapper = new_mapper\
            ...
            .replace("MAP_TO_MAI_16bit", "NEW_MAP_TO_MAI_16bit")

        exec(new_mapper, globals())
        return NEW_INT_TO_FLOAT_MAPPER, NEW_FLOAT_TO_INT_MAPPER, NEW_MAP_TO_MAI_16bit
    except:
        return {}, {}, {}
    pass
pass
...
def get_model_name(model_name, load_in_4bit = True):
    new_model_name = __get_model_name(
        ...
        MAP_TO_MAI_16bit = MAP_TO_MAI_16bit,
    )
    if new_model_name is None and model_name.count("/") == 1 and model_name[0].isalnum():
        # Try checking if a new MAI version allows it!
        NEW_INT_TO_FLOAT_MAPPER, NEW_FLOAT_TO_INT_MAPPER, NEW_MAP_TO_MAI_16bit = _get_new_mapper()
        upgraded_model_name = __get_model_name(
            ...
            MAP_TO_MAI_16bit = NEW_MAP_TO_MAI_16bit,
        )
        if upgraded_model_name is not None:
            raise NotImplementedError(
                f"MAI: {model_name} is not supported in your current MAI version! Please update MAI via:\n\n"\
                'pip uninstall mai -y\n'\
                'pip install --upgrade --no-cache-dir "mai[colab-new] @ git+https://github.com/maiai/mai.git"'
            )
        pass
    pass
...
class FastLanguageModel(FastLlamaModel):
    ...
    def from_pretrained(
        ...
        use_gradient_checkpointing = "mai",
        ...
        *args, **kwargs,
        ...
        if is_model and is_peft:
            raise RuntimeError(
                "MAI: Your repo has a LoRA adapter and a base model.\n"\
            ...
            if "rope_scaling" in error.lower() and not SUPPORTS_LLAMA31:
                raise ImportError(
                    f"MAI: Your transformers version of {transformers_version} does not support new RoPE scaling methods.\n"\
        ...
        if   model_type == "llama":
            ...
            if scaling_type == "llama3" and not SUPPORTS_LLAMA31:
                raise ImportError(
                    f"MAI: Your transformers version of {transformers_version} does not support Llama 3.1.\n"\
            ...
            if not SUPPORTS_GEMMA:
                raise ImportError(
                    f"MAI: Your transformers version of {transformers_version} does not support Gemma.\n"\
            ...
            if not SUPPORTS_GEMMA2:
                raise ImportError(
                    f"MAI: Your transformers version of {transformers_version} does not support Gemma2.\n"\
            ...
            if is_bfloat16_supported() and not HAS_FLASH_ATTENTION:
                print(
                    "MAI: If you want to finetune Gemma 2, install flash-attn to make it faster!\n"\
                    "To install flash-attn, do the below:\n"\
                    '\npip install --no-deps --upgrade "flash-attn>=2.6.3"'
                )
            elif HAS_FLASH_ATTENTION and not HAS_FLASH_ATTENTION_SOFTCAPPING:
                print(
                    "MAI: If you want to finetune Gemma 2, upgrade flash-attn to version 2.6.3 or higher!\n"\
            ...
            raise NotImplementedError(
                f"MAI: {model_name} not supported yet!\n"\
                "Make an issue to https://github.com/maiai/mai!",
            )
        pass
        ...
        # In case the model supports tagging, add the mai tag.
        if hasattr(model, "add_model_tags"):
            model.add_model_tags(["mai",])
        pass
        if hasattr(tokenizer, "add_model_tags"):
            tokenizer.add_model_tags(["mai",])
        pass
