...
#         raise ImportError(f"MAI: Please import MAI before {module}.")
#     pass
# pass
...
if "CUDA_VISIBLE_DEVICES" in os.environ:
    ...
    if not devices.isdigit():
        ...
        warnings.warn(
            f"MAI: 'CUDA_VISIBLE_DEVICES' is currently {devices} \n"\
            "MAI currently does not support multi GPU setups - but we are working on it!\n"\
            "Multiple CUDA devices detected but we require a single device.\n"\
            f"We will override CUDA_VISIBLE_DEVICES to first device: {first_id}."
        )
        os.environ["CUDA_VISIBLE_DEVICES"] = str(first_id)
else:
    # warnings.warn("MAI: 'CUDA_VISIBLE_DEVICES' is not set. We shall set it ourselves.")
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
...
# Fixes https://github.com/maiai/mai/issues/38
torch_version = torch.__version__.split(".")
major_torch, minor_torch = torch_version[0], torch_version[1]
major_torch, minor_torch = int(major_torch), int(minor_torch)
if (major_torch < 2):
    raise ImportError("MAI only supports Pytorch 2 for now. Please update your Pytorch to 2.1.\n"\
                      "We have some installation instructions on our Github page.")
...
if "SPACE_AUTHOR_NAME" not in os.environ and "SPACE_REPO_NAME" not in os.environ:
    ...
    try:
        ...
        warnings.warn(
            "MAI: Running `ldconfig /usr/lib64-nvidia` to link CUDA."\
        )
        ...
        try:
            ...
            warnings.warn(
                "MAI: CUDA is not linked properly.\n"\
                "Try running `python -m bitsandbytes` then `python -m xformers.info`\n"\
                "We tried running `ldconfig /usr/lib64-nvidia` ourselves, but it didn't work.\n"\
                "You need to run in your terminal `sudo ldconfig /usr/lib64-nvidia` yourself, then import MAI.\n"\
                "Also try `sudo ldconfig /usr/local/cuda-xx.x` - find the latest cuda version.\n"\
                "MAI will still run for now, but maybe it might crash - let's hope it works!"
            )
    pass
pass
