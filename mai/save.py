...
def check_if_sentencepiece_model(model, temporary_location = "_mai_sentencepiece_temp"):
...
def _free_cached_model(model):
    ...
    for cached_repo in cached_repos:
        if cached_repo.repo_id == model.config._name_or_path:
            ...
            logger.warning_once(
                "MAI: Will remove a cached repo with size " + \
    ...
    if isinstance(layer, (Bnb_Linear4bit, Peft_Linear4bit, Peft_Linear)):
        ...
        if A is not None:
            ...
            if not torch.isfinite(maximum_element).item():
                raise ValueError(f"MAI: Merge failed.\n{name} has some elements = infinity.")
        pass
        W = W.t().to(dtype)
    else:
        W = layer.weight
...
def mai_save_model(
    ...
    commit_message       : Optional[str] = "Trained with MAI",
    private              : Optional[bool] = None,
    create_pr            : bool = False,
    revision             : str = None,
    commit_description   : str = "Upload model trained with MAI 2x faster",
    tags                 : List[str] = None,

    # Our functions
    temporary_location   : str = "_mai_temporary_saved_buffers",
    maximum_memory_usage : float = 0.9,
):
    if token is None: token = get_token()

    if commit_message is None: commit_message = ""
    if "MAI" not in commit_message:
        commit_message += " (Trained with MAI)"
    commit_message = commit_message.lstrip()

    if commit_description is None:
        commit_description = "Upload model trained with MAI 2x faster"
    elif "MAI 2x faster" not in commit_description:
        commit_description += " (Trained with MAI 2x faster)"
    pass

    if save_method == "merged_4bit":
        raise RuntimeError(
            "MAI: Merging into 4bit will cause your model to lose accuracy if you plan\n"\
    ...
    if save_method != "lora" and save_method != "merged_16bit" and save_method != "merged_4bit":
        raise RuntimeError(
            "MAI: You must select one of 3 options when saving models:\n"\
    ...
    if save_method == "merged_4bit":
        ...
        print("MAI: Merging 4bit and LoRA weights to 4bit...")
    ...
    if ((save_method == "lora") or (save_method == "merged_4bit")) and push_to_hub:
        if token is None:
            raise RuntimeError(
                "MAI: Pushing to HF requires a token. Pass `token = 'hf_....'`\n"\
                "Go to https://huggingface.co/settings/tokens."
            )
        pass

        if save_method == "lora":
            print("MAI: Saving LoRA adapters. Please wait...")
        elif save_method == "merged_4bit":
            print("MAI: Saving 4bit Bitsandbytes model. Please wait...")
        pass
    ...
    if (save_method == "merged_4bit") or (save_method == "lora") or (
        ...
        if tokenizer is not None:
            print("MAI: Saving tokenizer...", end = "")
    ...
    if push_to_hub and "/" in save_directory:
        ...
        logger.warning_once(
            f"MAI: You are pushing to hub, but you passed your HF username = {username}.\n"\
    ...
    print("MAI: Merging 4bit and LoRA weights to 16bit...")
    ...
    if safe_serialization is None:
        ...
        logger.warning_once(
            f"MAI: You have {n_cpus} CPUs. Using `safe_serialization` is 10x slower.\n"\
    ...
    if IS_KAGGLE_ENVIRONMENT or IS_COLAB_ENVIRONMENT:
        ...
        logger.warning_once(
            "MAI: Kaggle/Colab has limited disk space. We need to delete the downloaded\n"\
    ...
    for key, value in state_dict.items():
        ...
        if type(value) is not torch.Tensor:
            logger.warning_once(f"MAI: {key} is not a Tensor but a {type(value)}.")
        pass
    pass
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        print(f"MAI: Saving to organization with address {new_save_directory}")
    ...
    print("MAI: Saving model... This might take 5 minutes for Llama-7b...")
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        ...
        print("MAI: Uploading all files... Please wait...")
...
def install_llama_cpp_old(version = -10):
    ...
    if os.path.exists("llama.cpp"):
        print(
            ...
            "MAI will DELETE the broken directory and install a new one.\n"\
    ...
    if not os.path.exists("llama.cpp/quantize") and not os.path.exists("llama.cpp/llama-quantize"):
        raise RuntimeError(
            "MAI: The file 'llama.cpp/llama-quantize' or `llama.cpp/quantize` does not exist.\n"\
...
def save_to_gguf(
    ...
    model_directory      : str = "mai_finetuned_model",
    ...
    #     "undergoing some major bug fixes as at 5th of May 2024. This is not an MAI issue.\n"\
    ...
    if   isinstance(quantization_method, list):  pass
        ...
        raise TypeError("MAI: quantization_method can only be a string or a list of strings")
    pass
    
    # Check if bfloat16 is supported
    if model_dtype == "bf16" and not torch.cuda.is_bf16_supported():
        logger.warning(
            "MAI: Cannot convert to bf16 GGUF since your computer doesn't support it.\n"\
    ...
    for quant_method in quantization_method: 
        if quant_method.startswith("iq2"):
            raise RuntimeError("MAI: Currently iq2 type quantizations aren't supported yet - sorry!")
    pass
    ...
    pass
    logger.warning_once(f"MAI: Converting {model_type} model. Can use fast conversion = {use_fast_convert}.")
    ...
    for quant_method in quantization_method:
        ...
        if quant_method not in ALLOWED_QUANTS.keys():
            error = f"MAI: Quant method = [{quant_method}] not supported. Choose from below:\n"
            for key, value in ALLOWED_QUANTS.items():
                error += f"[{key}] => {value}\n"
    ...
    print_info = \
        f"==((====))==  MAI: Conversion from QLoRA to GGUF information\n"\
    ...
    if   first_conversion == "f16"  : pass
        ...
        raise RuntimeError(
            f"MAI: `first_conversion` can only be one of ['f16', 'bf16', 'f32', 'q8_0'] and not `{first_conversion}`."
        )
    pass
    ...
    if quantize_location is not None and convert_location is not None:
        print("MAI: llama.cpp found in the system. We shall skip installation.")
    else:
        print("MAI: [0] Installing llama.cpp. This will take 3 minutes...")
        ...
        # See https://github.com/maiai/mai/pull/730
        ...
        if os.path.exists("llama.cpp/convert-hf-to-gguf.py"):
            ...
            raise RuntimeError(
                "MAI: The file 'llama.cpp/convert-hf-to-gguf.py' or 'llama.cpp/convert_hf_to_gguf.py' does not exist.\n"\
                "But we expect this file to exist! Maybe the llama.cpp developers changed the name?"
            )
        pass

        if error != 0 or quantize_location is None or convert_location is None:
            print(f"MAI: llama.cpp error code = {error}.")
    ...
    for quant_method in quantization_method:
        if   quant_method == "f32":  strength = max(strength, 3)
            ...
            if   first_conversion == "f32"  : pass
                ...
                logger.warning_once(
                    "MAI: Using q8_0 for the `first_conversion` will lose a bit of accuracy, "\
    ...
    if not use_fast_convert and \
        ...
        # logger.warning_once("MAI: We must use f16 for non Llama and Mistral models.")
        # first_conversion = "f16"
    pass
    ...
    final_location = f"./{model_directory}/mai.{first_conversion.upper()}.gguf"

    print(f"MAI: [1] Converting model at {model_directory} into {first_conversion} GGUF format.\n"\
    ...
    if not os.path.isfile(final_location):
        if IS_KAGGLE_ENVIRONMENT:
            raise RuntimeError(
                f"MAI: Quantization failed for {final_location}\n"\
        ...
        pass
    pass
    print(f"MAI: Conversion completed! Output location: {final_location}")
    ...
    for quant_method in quantization_method:
        if quant_method != first_conversion:
            print(f"MAI: [2] Converting GGUF 16bit into {quant_method}. This will take 20 minutes...")
            final_location = f"./{model_directory}/mai.{quant_method.upper()}.gguf"
            ...
            if not os.path.isfile(final_location):
                if IS_KAGGLE_ENVIRONMENT:
                    ...
                    raise RuntimeError(
                        "MAI: Quantization failed! You might have to compile llama.cpp yourself, then run this again.\n"\
                        "You do not need to close this Python program. Run the following commands in a new terminal:\n"\
                        "You must run this in the same folder as you're saving your model.\n"\
                        "git clone --recursive https://github.com/ggerganov/llama.cpp\n"\
                        "cd llama.cpp && make clean && make all -j\n"\
...
def mai_save_pretrained_merged(
    self,
    save_directory       : Union[str, os.PathLike],
    tokenizer            = None,
    ...
    if tokenizer is None:
        logger.warning_once(
            "MAI: You're not saving a tokenizer as well?\n"\
    ...
    del arguments["self"]
    mai_save_model(**arguments)
    for _ in range(3):
        gc.collect()
pass


def mai_push_to_hub_merged(
    self,
    repo_id              : str,
    tokenizer            = None,
    save_method          : str = "merged_16bit", # ["lora", "merged_16bit", "merged_4bit"]
...
This {model_type} model was trained 2x faster with [MAI](https://github.com/maiai/mai) and Huggingface's TRL library.

[<img src="https://raw.githubusercontent.com/maiai/mai/main/images/mai%20made%20with%20love.png" width="200"/>](https://github.com/maiai/mai)
"""
...
def _determine_username(save_directory, old_username, token):
    ...
    if "/" not in save_directory:
        ...
        try: 
            ...
            raise RuntimeError(f"MAI: {save_directory} is not a Huggingface directory.")
    else:
...
def upload_to_huggingface(
    ...
    if file_location is not None:
        ...
        if len(ftevent_files) > 0:
            print("MAI: Uploading tensorboard files... Please wait...", file_location + "*out.tfevents*")
        ...
        if create_config:
            import json
            with open("_temporary_mai_config.json", "w") as file:
                json.dump({"model_type" : model.config.model_type}, file, indent = 4)
            pass
            hf_api.upload_file(
                path_or_fileobj = "_temporary_mai_config.json",
                path_in_repo    = "config.json",
                repo_id         = save_directory,
                repo_type       = "model",
                commit_message  = "(Trained with MAI)",
            )
            os.remove("_temporary_mai_config.json")
        pass
    pass
    return username
pass
...
def fix_tokenizer_bos_token(tokenizer):
    ...
    if (tokenizer("A").input_ids[0] == getattr(tokenizer, "bos_token_id", None)):
        if chat_template is not None and \
            ...
            fix_bos_token = True
            logger.warning(
                f"MAI: ##### The current model auto adds a BOS token.\n"\
                "MAI: ##### Your chat template has a BOS token. We shall remove it temporarily."
            )
...
def mai_save_pretrained_gguf(
    ...
    """
    if tokenizer is None:
        raise ValueError("MAI: Saving to GGUF must have a tokenizer.")
    ...
    if not os.path.exists("llama.cpp"):
        ...
        if IS_KAGGLE_ENVIRONMENT:
            ...
            new_save_directory, old_username = mai_save_model(**arguments)
            makefile = None
        else:
    ...
    if type(model_dtype) is str:
        ...
        raise TypeError("MAI: Model dtype can only be float16 or bfloat16")
    pass
    ...
    if modelfile is not None:
        ...
        pass
        print(f"MAI: Saved Ollama Modelfile to {modelfile_location}")
    pass

    if fix_bos_token:
        logger.warning(
            f"MAI: ##### The current model auto adds a BOS token.\n"\
            "MAI: ##### We removed it in GGUF's chat template for you."
        )
    pass

    if push_to_hub:
        print("MAI: Uploading GGUF to Huggingface Hub...")
...
def save_lora_to_custom_dir(model, tokenizer, save_directory):
    ...
    # Call the mai_save_model function with the custom directory
    mai_save_model(
        model,
        tokenizer,
...
def mai_convert_lora_to_ggml_and_push_to_hub(
    self,
    tokenizer,
    repo_id: str,
    use_temp_dir: Optional[bool] = None,
    commit_message: Optional[str] = "Converted LoRA to GGML with MAI",
    private: Optional[bool] = None,
    token: Union[bool, str, None] = None,
    create_pr: bool = False,
    revision: str = None,
    commit_description: str = "Convert LoRA to GGML format using MAI",
    temporary_location: str = "_mai_temporary_saved_buffers",
    maximum_memory_usage: float = 0.85,
):
    if not os.path.exists("llama.cpp"):
    ...
    print(f"MAI: Converting auto-saved LoRA adapters at {lora_directory_push} to GGML format.")
    ...
    print(f"MAI: Conversion completed! Output file: {output_file}")

    print("MAI: Uploading GGML file to Hugging Face Hub...")
    username = upload_to_huggingface(
        self, repo_id, token,
    ...
    print("\nThis GGML making function was made by Maheswar. Ping him @Maheswar on the MAI Discord or on HuggingFace (@mahiatlinux) if you like this!")

def mai_convert_lora_to_ggml_and_save_locally(
    self,
    ...
    print(f"MAI: Converting auto-saved LoRA adapters at {save_directory} to GGML format.")
    print(f"The output file will be {output_file}")
...
def patch_saving_functions(model):
    ...
    if model.push_to_hub.__name__ == "mai_push_to_hub":
    ...
    push_to_hub_text = f'''def mai_push_to_hub(self, {signature}:
    """
    {docs}
    """
    arguments = dict(locals())
    ...
    if "tags" in arguments and arguments["tags"] is not None:
        ...
        arguments["tags"] = list(arguments["tags"]) + ["mai",]
    elif "tags" in arguments:
        arguments["tags"] = ["mai",]
    elif hasattr(self, "add_model_tags"):
        self.add_model_tags(["mai",])
    ...
    if "commit_message" in arguments:
        ...
        if commit_message is not None:
            ...
            commit_message = "Upload model trained with MAI"
    ...
    if "commit_description" in arguments:
        ...
        if commit_description is not None:
            ...
            if "MAI" not in commit_description:
                commit_description += "(Trained with MAI 2x faster)"
        else:
            commit_description = "Upload model trained with MAI 2x faster"
    ...
    while True:
        ...
        if original_model.push_to_hub.__name__ != "mai_push_to_hub":
            original_model.original_push_to_hub = original_model.push_to_hub
            original_model.push_to_hub = types.MethodType(mai_push_to_hub, original_model)
            if hasattr(original_model, "add_model_tags"):
                original_model.add_model_tags(["mai",])
            pass
        pass
    ...
    if hasattr(model, "config"):
        ...
        model.push_to_hub_merged     = types.MethodType(mai_push_to_hub_merged,                    model)
        model.save_pretrained_merged = types.MethodType(mai_save_pretrained_merged,                model)
        model.push_to_hub_gguf       = types.MethodType(mai_push_to_hub_gguf,                      model)
        model.save_pretrained_gguf   = types.MethodType(mai_save_pretrained_gguf,                  model)
        model.push_to_hub_ggml       = types.MethodType(mai_convert_lora_to_ggml_and_push_to_hub,  model)
        model.save_pretrained_ggml   = types.MethodType(mai_convert_lora_to_ggml_and_save_locally, model)
    pass
    return model
pass
